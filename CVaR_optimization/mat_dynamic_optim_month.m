fprintf("CVaR optimization for Naive-GRU model with Dow dataset\n");
%% exit with[nohup ./matlab -nosplash -nodisplay -nodesktop < CVaR_optimization/mat_dynamic_optim_month.m > data/cvar_ouput/result_NN_month_time_att.log 2>&1 &]
...
% 导入数据
parammatrixSgamma = readtable("data/gru_save/param_matrix_S_gamma_step_one_ahead_month_time_att.csv", opts);
...
% 导入数据
parammatrixSbeta = readtable("data/gru_save/param_matrix_S_beta_step_one_ahead_month_time_att.csv", opts);
...
% 导入数据
parammatrixSalpha = readtable("data/gru_save/param_matrix_S_alpha_step_one_ahead_month_time_att.csv", opts);
...
tl1= 1+1*(2611+652-1)+200; %% start index of test date
interval = 21;
TK = T;
B = 10;
...
writematrix(return_mat,'data/cvar_ouput/naive_cumulation.csv');
