fprintf("DCC-DM model training and CVaR optimization. \n");
%% exit with[nohup ./matlab -nosplash -nodisplay -nodesktop < CVaR_optimization/Dcc_fitting_month.m > data/cvar_ouput/result_dcc_dm.log 2>&1 &]
...
mean_all = mean(dailyreturn{:,2:end},1);
std_all = std(dailyreturn{:,2:end},1);
...
YMt = (hist_mat(:,1)-mean_all(1))/std_all(1);
...
for kk=1:K
    YMt = (hist_mat(:,1)-mean_all(1))/std_all(1);
    Yit = (hist_mat(:,kk+1)-mean_all(kk+1))/std_all(kk+1);
...
for j = 1:TK
 ...
 for i_m = 1:length(beta_M_seq(1,:))
    ...
    ZM=normrnd(0,1,[M*B,interval]);
    sigma_M_month = ones(M*B,interval);
    sigma_M_month(:,1) = sigma_M_month(:,1)*sigma_M_start;

    YMat = zeros(M*B,K);
    for kk =1:K
      sigma_S_month = ones(M*B,interval);
      sigma_S_month(:,1) = sigma_S_month(:,1)*theta_start(1,kk);

      rho_S_month = ones(M*B,interval);
      rho_S_month(:,1) = rho_S_month(:,1)*theta_start(2,kk);
      Zi=normrnd(0,1,[M*B,interval]);
      mon_temp = ones(M*B,interval);
      mon_temp(:,1) =  sqrt(theta_start(1,kk))*(ZM(:,1) *theta_start(2,kk)+Zi(:,1) *sqrt(1-theta_start(2,kk)*theta_start(2,kk)));

     for ii= 2:interval

        sigma_M_month(:,ii) = parameters_M(1)+(parameters_M(2)* (ZM(:,ii-1)).^2+ parameters_M(3)).* sigma_M_month(:,ii-1);
        sigma_S_month(:,ii) = parammatrix(1,kk)+parammatrix(2,kk)* (mon_temp(:,ii-1)).^2+ parammatrix(3,kk)* sigma_S_month(:,ii-1);

        cond_Y = ZM(:,ii-1) .*rho_S_month(:,ii-1)+Zi(:,ii-1) .*sqrt(1-rho_S_month(:,ii-1).*rho_S_month(:,ii-1));
        Q_pred =(1-parammatrix(5,kk)-parammatrix(6,kk))* parammatrix(4,kk)+parammatrix(5,kk)*(ZM(:,ii-1).*cond_Y)+parammatrix(6,kk)*rho_S_month(:,ii-1);
        Q_pred = Q_pred./sqrt(1-parammatrix(5,kk)+parammatrix(5,kk)*(ZM(:,ii-1).*ZM(:,ii-1)));
        Q_pred = Q_pred./sqrt(1-parammatrix(5,kk)+parammatrix(5,kk)*(cond_Y.*cond_Y));
        rho_S_month(:,ii) = Q_pred;

        mon_temp(:,ii) =  sqrt(sigma_S_month(:,ii)).*(ZM(:,ii) .*rho_S_month(:,ii)+Zi(:,ii) .*sqrt(1-rho_S_month(:,ii).*rho_S_month(:,ii)));


     end
      mon_temp = mon_temp*std_all(kk+1)+mean_all(kk+1);
      YMat(:,kk) =  prod(mon_temp+1,2)-1;
...
%%
writematrix(YMat_TK,'data/cvar_ouput/Simu_return_mat_dcc_dynamic_month.csv');
